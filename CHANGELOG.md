...
- **Deterrent popups self-destructing immediately** — when a deterrent popup window opened, Chrome fired `onActivated` for the tab inside it; `handleActivated` interpreted this as the user switching away from the blocked tab and called `stopBlocking`, which closed all three popup windows within a second of them opening. Fixed by tracking which window IDs are deterrents (`deterrentWindowIds`) and skipping the `stopBlocking` call when the newly-activated tab belongs to one of them.

- **Tab blocker never firing while browsing** — block decisions were only evaluated when the extension sent a tab event, which only happened on tab switch or page load. Staying on a social media page with no navigation meant the limit could be exceeded but the block overlay never appeared. Fixed by adding a 15-second heartbeat in the Chrome extension service worker that re-sends the current active tab while the WebSocket connection is open, triggering a fresh block evaluation on a regular interval.
